...
export async function POST(request: NextRequest) {
  try {
    ...
    const visitorData = {
      ip: (request as any).ip || request.headers.get('x-forwarded-for')?.split(',')[0] || 'unknown',
      userAgent: request.headers.get('user-agent') || metadata.userAgent || 'unknown',
    ...
    if (API_BASE_URL) {
      try {
        ...
        if (!response.ok) {
          console.error('Failed to send analytics to backend:', response.status);
        }
      } catch (error) {
      ...
      console.warn('DAYSYNC_API_BASE_URL not configured - analytics data logged locally only');
    }
